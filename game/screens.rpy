...
init python:
    def FinishEnterName(launchGame=True):
        ...
        if persistent.playername == "iwannasayori":
            persistent.playername = persistent.usr
            renpy.jump_out_of_context("sayori")

        if persistent.playername == "justmonika":
            persistent.playername = persistent.usr
            #renpy.jump_out_of_context("monika")

        if persistent.playername == "iwannacake":
            persistent.playername = persistent.usr
            #renpy.jump_out_of_context("natsuki")

        if persistent.playername == "iwannaknife":
            persistent.playername = persistent.usr
            #renpy.jump_out_of_context("natsuki")

        if persistent.playername == "developer":
            persistent.playername = persistent.usr
            renpy.jump_out_of_context("gelistirici")
